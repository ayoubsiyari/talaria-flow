...
function PlaybackBar({ time, duration, playing, onPlayPause, onReset, onSeek, onHover }) {
  ...
  return (
    <div data-omelette-chrome style={{
      ...
      <IconButton onClick={onPlayPause} label="Play/pause (space)">
        {playing ? (
      ...
      <IconButton onClick={onReset} label="Return to start (0)">
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
function wcWarnOnce(key, message, err) {
  ...
  wcWarned[key] = true;
  try {
    if (window.TF && window.TF.captureException) window.TF.captureException(err instanceof Error ? err : new Error(message), 'tutorial');
  } catch (e) {}
}
