...
- `templates.json` — the nine templates. Each has `file`, `name`, `trigger`, `kind` (`auto` | `campaign`), `subject`, `preheader`, `eyebrow`, `title`, optional `accent` (`#2EE8FF` default · `#FBBF24` under review · `#FF37B0` rejection/launch), `unsubscribe: false` on every transactional template (01–05, 07), and `blocks[]`. Seeded into `public.email_templates` by `scripts/seed.ts` (`supabase/seed.sql` is generated from it); `pnpm build` copies it to `public/assets/emails/templates.json` for the admin preview.

| # | id | Sent by | Merge fields |
|---|---|---|---|
| 01 | confirm-email | Supabase Auth "Confirm signup" (paste-in from `supabase/templates/`) | email, token |
| 02 | signup-code | Supabase Auth "Magic link / OTP" | code, device, location, time |
| 03 | submission-received | `POST /api/proof` | file_count, submitted_at |
| 04 | approved | `POST /api/admin/submission` (approve) | — |
| 05 | needs-resubmission | `POST /api/admin/submission` (reject) | reason |
| 06 | course-ready | campaign (`POST /api/admin/campaigns`), pref `course` | note |
| 07 | password-reset | Supabase Auth "Reset password" | email, token |
...
Always available: `email`, `account_email`, `first_name`, `dashboard_url`, `unsubscribe_url`, `preferences_url`. Merge fields are `{{snake_case}}` and are substituted after render, per recipient, HTML-escaped. `unsubscribe_url` is a signed one-click link (`/api/unsubscribe`, see `src/lib/server/unsubscribe.ts`) for the preference that matches the template (06 → course, 08 → newsletter, 09 → tools; waitlist recipients → removal from the waitlist); `preferences_url` is `/account/notifications/`.
...
01, 02 and 07 are sent by Supabase Auth, not by the site. Copies with the Supabase placeholders (`{{ .ConfirmationURL }}`, `{{ .Token }}`, `{{ .Email }}`) already substituted live in `supabase/templates/`; paste them into Supabase → Authentication → Email Templates (DEPLOY.md §1.3).
