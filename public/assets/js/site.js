  ...
  function consentCopy(lang) {
    ...
    return {
      body: pick('cookies.body', ar
        ? 'نستخدم ملفات تعريف الارتباط الضرورية لتشغيل الموقع وتذكّر لغتك. لا نستخدم إعلانات ولا تتبّعاً'
        : 'We use cookies that are needed to run the site and remember your language. No advertising, no tracking.'),
      accept: pick('cookies.ok', ar ? 'موافق' : 'Accept'),
      reject: pick('cookies.reject', ar ? 'الضروري فقط' : 'Essential only'),
  ...
  function showConsent(force) {
    ...
    bar.innerHTML =
      ...
      '<div style="display:flex;gap:10px;flex:none">' +
        '<button type="button" data-consent="essential" style="height:40px;padding:0 16px;border-radius:10px;border:1px solid rgba(255,255,255,0.16);background:transparent;color:#F2F4F8;font:600 14px ' + (ar ? 'Cairo' : 'Archivo') + ',sans-serif;cursor:pointer;white-space:nowrap;line-height:1">' + t.reject + '</button>' +
        '<button type="button" data-consent="all" class="btn-outline" style="height:40px;padding:0 18px;border-radius:10px;border:1px solid #2EE8FF;background:transparent;color:#2EE8FF;font:600 14px ' + (ar ? 'Cairo' : 'Archivo') + ',sans-serif;cursor:pointer;white-space:nowrap;line-height:1">' + t.accept + '</button>' +
      '</div>';
